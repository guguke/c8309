		      ...
		      struct can_bittime_std *bit_time)
{
	...
	printk(" func:(%s) baudrate:(%08x %d) can_sys_clock:%llu \n", __FUNCTION__,baudrate,baudrate,(u64)can->can_sys_clock);
	/* baudrate range [1baud,1Mbaud] */
	//if (baudrate == 0 || baudrate > 1000000UL) return -EINVAL;

	tmp = (u64)can->can_sys_clock*1000;
	printk(" func:(%s) baudrate:(%08x %d) tmp.u64:%llu \n", __FUNCTION__,baudrate,baudrate,tmp,tmp);
	do_div(tmp, baudrate);
	...
	brp_max = (brp_expected + 500 * MIN_BIT_TIME) / (1000 * MIN_BIT_TIME);
        printk(" func:(%s)   brp_min:%u brp_max:%u brp_expected:%u \n",__FUNCTION__,brp_min,brp_max,brp_expected);
	if (brp_max == 0)
		brp_max = 1;
	...
	for (brp = brp_min; brp <= brp_max; brp++) {
		quanta = brp_expected / (brp * 1000);
                printk("   quanta : %d max:%d min:%d \n",quanta, MAX_BIT_TIME,MIN_BIT_TIME );
...
static int can_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	printk(" func:(%s) 0x%08x %p  baudrate:(%lu) \n", __FUNCTION__, cmd, &ifr->ifr_ifru,*baudrate);
