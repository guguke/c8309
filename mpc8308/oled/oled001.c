//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
//  UG-2864KLBMG01 (2864-95E) Reference Code
//
//    Dot Matrix: 128*64
...
//  Pin Definition
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#define SCL	P1_0				// Serial Clock Input
#define SDA	P1_1				// Serial Data Input
						//   D1/D2 should be tied together.

#define RES	P3_3				// Reset

#define DC	P3_2				// Slave Address
						//   DC=0 => 0x78 (Write Mode) / 0x79 (Read Mode)
						//   DC=1 => 0x7A (Write Mode) / 0x7B (Read Mode)

						// BS1=1; BS2=0
						// The unused pins should be connected with VSS.
						// Please refer to the SSD1309 specification for detail.

int getipmask(char *iface,char *ipmask,char *szip,char *szmask)
{
	int fd;
	struct ifreq ifr;
	//char szip[100],szmask[100];
	...
	sprintf(szip,"%s" , inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) );
	//get the netmask ip
	ioctl(fd, SIOCGIFNETMASK, &ifr);
	//display netmask
	printf("Netmask of %s - %s\n" , iface , inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) );
	sprintf(szmask,"%s" , inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr) );
	close(fd);
	sprintf(ipmask,"%s/%s",szip,szmask);
	return 0;
}
int a2n(int a)
{
...
{
	char cmd[200];
	sprintf(cmd,"/usr/sbin/i2cset -y 0 0x3c 0x00 0x%02x",Data);
	system(cmd);
#if 0
	I2C_Start();
	I2C_O(0x00);
...
{
	char cmd[200];
	sprintf(cmd,"/usr/sbin/i2cset -y 0 0x3c 0x40 0x%02x",Data);
	system(cmd);
#if 0
	I2C_Start();
	I2C_O(0x40);
...
unsigned char i;
	...
	Set_Display_On_Off(0xAE);		// Display Off (0xAE/0xAF)
	Set_Display_Clock(0xA0);		// Set Clock as 100 Frames/Sec
...
void main()
{
	...
	// +886-37-587168
	unsigned char i;
	unsigned char ipmask[100],ipn[100];
	unsigned char szip[100],szipn[100],szmask[100],szmaskn[100];
	int len;

	getipmask("eth0",ipmask,szip,szmask);
	len=strlen(ipmask);
	for(i=0;i<len;i++) ipn[i]=a2n(0x0ff & ipmask[i]);
	ipn[len]=0;
	len=strlen(szip);
	for(i=0;i<len;i++) szipn[i]=a2n(0x0ff & szip[i]);
	szipn[len]=0;
	len=strlen(szmask);
	for(i=0;i<len;i++) szmaskn[i]=a2n(0x0ff & szmask[i]);
	szmaskn[len]=0;

	//P1=0xFF;
	//P3=0xFF;

	//DC=0;					// Set Slave Address
	OLED_Init();
#if 0
	//while(1)
	//{
	...
	//}
#endif
		Show_String(1,szipn,0x05,XLevel+0x16);
		Show_String(1,szmaskn,0x06,XLevel+0x16);
}
