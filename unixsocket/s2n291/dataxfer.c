...
init_port_data(port_info_t *port)
{
    ...
    port->bt.hexdump = 0;
	// cyx
	printf("   mydebug init_port_data  portname:%s  \n",port->portname);
}
...
int myport2sn(char *pname)
{
	int sn;
	sn=atoi(pname);
	switch(sn){
	case 3000:sn=0;break;
	case 3001:sn=1;break;
	case 3002:sn=2;break;
	case 3003:sn=3;break;
	case 3004:sn=4;break;
	case 3005:sn=5;break;
	case 3006:sn=6;break;
	case 3007:sn=7;break;
	default:sn=0;break;
	}
	return sn;
}
...
// report serial n para 
int udp_a010(char *pname)
{
	char pout[30];
	char udpip[40];
	int udpport;
	...
	pout[13]=mycrc(pout);
	len = 14;
	udp_broadcast_send(udpip,udpport,pout,len,1);//addr,nport,buf,len,nn);
...
// stat : 
// 0x00 not connect == 
// 0x10 connect,no data  == 
// 0x11,connect,data>0
int myChangeStat(int stat,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	printf(" mydebug , mychangestat portname:%s sn:%d    0x%02x ==> 0x%02x \n",
		pname, sn, gport_stat[sn].stat, stat);
	gport_stat[sn].stat=stat;//
	udp_af20();
	return 0;
}
...
handle_dev_fd_read(int fd, void *data)
{
	...
	// cyx
	if(count>0 && port->dev_bytes_sent==0 && port->dev_bytes_received ==0 ){
		printf(" mydebug , 001 port:%s  status change , 0x10 ==> 0x11 \n",port->portname);
		myChangeStat(0x11,port->portname);
	}
...
retry_write:
	...
	if (count == -1) {
		...
		// cyx
		if(count>0 && port->dev_bytes_sent==0 && port->dev_bytes_received ==0 ){
			printf(" mydebug , 000 port:%s  status change , 0x10 ==> 0x11 \n",port->portname);
			myChangeStat(0x11,port->portname);
		}
...
void myinit_gd(char *pname)
{
	...
	i=atoi(pname);
	i=(i%10)%8;
	printf(" mydebug myinit_gd   portname:%s   sn:%d\n",pname,i);
...
int mySaveIpRemote(port_info_t *port)
{
	...
	sn=myport2sn(port->portname);
	gport_stat[sn].ip=inp.s_addr;
...
setup_tcp_port(port_info_t *port)
{
	...
	printf(" mydebug ,==000=== setup_tcp_port  portname : %s \n",port->portname);
	...
	myinit_gd(port->portname);
	port->devfd = open(port->devname, options);         /// cyx
	if (port->devfd == -1) {
		close(port->tcpfd);
		port->tcpfd = -1;
    ...
    reset_timer(port);
	// cyx
	printf(" mydebug open(devname) end of setup_tcp_port() portname:%s\n",port->portname);
	myChangeStat(0x10,port->portname);
	mySaveIpRemote(port);
...
finish_shutdown_port(port_info_t *port)
{
	...
	port->dev_bytes_sent = 0;
	// cyx
	printf("   mydebug 002 finish_shutdown_port \n");
	myChangeStat(0x00,port->portname);
	...
	// cyx
	//printf("   mydebug finish_shutdown_port \n");
	//myChangeStat(0x00,port->portname);
}
...
showshortport(struct controller_info *cntlr, port_info_t *port)
{
    ...
    getnameinfo((struct sockaddr *) &(port->remote), sizeof(port->remote),
		    ...
		    NI_NUMERICHOST | NI_NUMERICSERV);
    count = strlen(buffer);
	printf(" mydebug    0000    showshortport             buffer:%s \n",buffer);
    controller_output(cntlr, buffer, count);
    sprintf(buffer, ",%s ", portbuff);
	printf(" mydebug    0001    showshortport             buffer:%s \n",buffer);
    count += strlen(buffer);
    controller_output(cntlr, buffer, strlen(buffer));
    while (count < 23) {
...
int baud2n(speed_t baud)
{
	int ret;

	switch(baud){
	case B300: ret=0x0;break;
	case B600: ret=0x01;break;
	case B1200: ret=0x02;break;
	case B2400: ret=0x03;break;
	case B4800: ret=0x04;break;
	case B9600: ret=0x05;break;
	case B19200: ret=0x06;break;
	case B38400: ret=0x07;break;
	case B115200: ret=0x08;break;
	default:
		ret=0x08;
		break;
	}
...
void prnspeed(speed_t s,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	printf(" mydebug , prnspeed , portname:%s sn:%d \n",pname,sn);
	switch(s){
	case B115200:
		printf(" mydebug prnspeed , baudrate 115200 (%d)\n",s);
		break;
	case B9600:
		printf(" mydebug prnspeed , baudrate 9600 \n");
		break;
	case B19200:
		printf(" mydebug prnspeed , baudrate 19200 \n");
		break;
	default:
		printf(" mydebug prnspeed , not valid speed \n");
		break;
	}
	gport_stat[sn].baud=baud2n(s);
}
void mysavebits(int v,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	printf(" mydebug , mysavebits , portname:%s sn:%d    bits:%d\n",pname,sn,v);
	gport_stat[sn].bits=bits2n(v);
}
void mysaveparity(int v,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	switch(v){
	case 1:	gport_stat[sn].parity=0;break;//////// none
	case 2:	gport_stat[sn].parity=1;break;//////// odd
	case 3:	gport_stat[sn].parity=2;break;//////// even 
	default: gport_stat[sn].parity=0;break;//////// none
	}
	printf(" mydebug , mysave parity , portname:%s sn:%d    %d => %d \n",pname,sn,v,gport_stat[sn].parity);
}
void mysavestop(int v,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	switch(v){
	case 1:	gport_stat[sn].stop=1;break;//////// 1 bit
	case 2:	gport_stat[sn].stop=2;break;//////// 2 bit
	default: gport_stat[sn].stop=1;break;//////// 1 bit
	}
	printf(" mydebug , mysave stop , portname:%s sn:%d    %d => %d \n",pname,sn,v,gport_stat[sn].stop);
	udp_a010(pname);
}
void mysaveflowc(int v,char *pname)
{
	int sn;
	sn=atoi(pname);
	sn=(sn%10)%8;
	switch(v){
	case 10:	gport_stat[sn].flowctrl=0;break;//////// no 
	case 11:	gport_stat[sn].flowctrl=2;break;//////// xon
	case 12:	gport_stat[sn].flowctrl=1;break;//////// rtscts
	case 14:	gport_stat[sn].flowctrl=0;break;//////// no 
	case 15:	gport_stat[sn].flowctrl=2;break;//////// xon
	case 16:	gport_stat[sn].flowctrl=1;break;//////// rtscts
	case 1:	gport_stat[sn].flowctrl=0;break;//////// no 
	case 2:	gport_stat[sn].flowctrl=2;break;//////// xon
	case 3:	gport_stat[sn].flowctrl=1;break;//////// rtscts
	default:	gport_stat[sn].flowctrl=0;break;//////// no 
	}
	printf(" mydebug , mysave flowc , portname:%s sn:%d    %d => %d \n",pname,sn,v,gport_stat[sn].flowctrl);
	gport_stat[sn].flowctrl=0;//////// no 
}
static void
com_port_handler(void *cb_data, unsigned char *option, int len)
{
	...
	printf(" mydebug , com_port_handler, option[1]:%d    portname:%s\n",option[1],port->portname);
	switch (option[1]) {
	case 0: /* SIGNATURE? */
		{
	...
	case 3: /* SET-PARITY */
		...
		mysaveparity(val,port->portname);
		outopt[0] = 44;
		outopt[1] = 103;
		outopt[2] = val;
		telnet_send_option(&port->tn_data, outopt, 3);
	...
	case 5: /* SET-CONTROL */
		...
		printf(" mydebug , com_port_handler , option[2]:%d \n",option[2]);
		switch (option[2]) {
		case 0:
		case 1:
		case 2:
...
read_break_val:
			if (port->break_set)
				...
				val = 6;
			printf(" end of open serial port setattr == portname:%s\n",port->portname);
			break;
...
read_rts_val:
			if (ioctl(port->devfd, TIOCMGET, &val) == -1)
				...
				val = 12;
			mysaveflowc(val,port->portname);
			printf(" end of =close= serial port setattr == portname:%s\n",port->portname);
			break;
		}
