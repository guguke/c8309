...
static int cpld_pic_get_irq(int offset, u8 ignore, u8 __iomem *statusp, u8 __iomem *maskp)
{
	int cpld_irq;
...
static void cpld_pic_cascade(unsigned int irq, struct irq_desc *desc)
{
	irq = cpld_pic_get_irq(0, PCI_IGNORE, &cpld_regs->pci_status, &cpld_regs->pci_mask);
	if (irq != NO_IRQ && irq != NO_IRQ_IGNORE) {
		generic_handle_irq(irq);
		return;
	}

	irq = cpld_pic_get_irq(8, MISC_IGNORE, &cpld_regs->misc_status, &cpld_regs->misc_mask);
	if (irq != NO_IRQ && irq != NO_IRQ_IGNORE) {
		generic_handle_irq(irq);
		return;
	}
...
static int cpld_pic_host_match(struct irq_host *h, struct device_node *node)
{
...
void __init mpc5121_ads_cpld_pic_init(void)
{
	...
	/*
	 * statically route touch screen pendown through 1 and ignore it here
	 * route all others through our cascade irq
	 */
	out_8(&cpld_regs->route, 0x10);
