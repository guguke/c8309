              ...
              off_t offset, int buffer_length, int *eof, void *data)
{
        ...
        if (offset > 0) {
        //printk(KERN_INFO "offset(0x%08x) > 0 procfile_read (/proc/%s) called\n",(int)offset, PROCFS_NAME);
        ...
        //printk(KERN_INFO "offset(0x%08x) <=0  procfile_read (/proc/%s) called\n",(int)offset, PROCFS_NAME);
			    ...
			    procfs_buffer[0]='0';
			    procfs_buffer[1]='x';
			    procfs_buffer[2]=ascii_tbl[(v>>28)&0x0f];
			    procfs_buffer[3]=ascii_tbl[(v>>24)&0x0f];
			    procfs_buffer[4]=ascii_tbl[(v>>20)&0x0f];
			    procfs_buffer[5]=ascii_tbl[(v>>16)&0x0f];
			    procfs_buffer[6]=ascii_tbl[(v>>12)&0x0f];
			    procfs_buffer[7]=ascii_tbl[(v>>8)&0x0f];
			    procfs_buffer[8]=ascii_tbl[(v>>8)&0x0f];
			    procfs_buffer[9]=ascii_tbl[v&0x0f];
			    procfs_buffer[10]='\n';
			    procfs_buffer[11]=0;
